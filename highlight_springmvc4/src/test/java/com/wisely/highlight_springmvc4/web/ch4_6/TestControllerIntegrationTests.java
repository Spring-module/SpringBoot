...
/**
 * 测试用例：
 * 1 @WebAppConfiguration 注解在类上，用来声明加载的 ApplicationContext 是一个 WebApplicationContext。它的属性指定的是 Web 资源的位置，默认为 src/main/webapp，本例修改为 src/main/resources。
 * 2 MockMvc-模拟 MVC 对象，通过 MockMvcBuilders.webAppContextSetup(this.wac).build() 初始化。
 * 3 可以在测试用例中注入 Spring 的 Bean。
 * 4 可注入 WebApplicationContext。
 * 5 可注入模拟的 Http session，此处仅作演示，没有使用。
 * 6 可注入模拟的 http request，此处仅作演示，没有使用。
 * 7 @Before 在测试开始前进行的初始化工作。
 * 8 模拟向/normal 进行 get请求。
 * 9 预期控制返回状态为 200。
 * 10 预期 view 的名称为 page。
 * 11 预期页面转向的真正路径为/WEB-INF/classes/view/page.jsp。
 * 12 预期 Model 里的值是 demoService.saySomething() 返回值 hello。
 * 13 模拟向 /testRest 进行 get 请求。
 * 14 预期返回值的媒体类型为 text/plain;charset=UTF-8。
 * 15 预期返回值的内容为 demoService.saySomething() 返回值 hello。
 */
